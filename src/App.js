...
function EmailCapture({ onClose, restoreMode }) {
  ...
  // GitHub OAuth needs no email deliverability at all — this is the reliable
  // sign-in path while custom SMTP isn't set up yet (Supabase's default
  // mailer only delivers to project team members, so magic links above won't
  // reach real users until a verified sending domain is connected).
  const submitGitHub = async () => {
    ...
    if (oauthError) setError("Couldn't start GitHub sign-in. Try again in a moment.");
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#0d0d0d", border: "1px solid #00ff8830", borderRadius: "16px", width: "100%", maxWidth: "440px", padding: "28px 24px", fontFamily: "'Space Mono', monospace" }}>
        ...
        <button onClick={submitGitHub} disabled={oauthSending || sending} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", background: "#fff", color: "#000", border: "none", borderRadius: "8px", padding: "13px", cursor: oauthSending ? "wait" : "pointer", fontWeight: "bold", fontSize: "14px", fontFamily: "'Space Mono', monospace", marginBottom: "16px", opacity: oauthSending ? 0.7 : 1 }}>
          <svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
          {oauthSending ? "Redirecting..." : "Continue with GitHub"}
        </button>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}>
          <div style={{ flex: 1, height: "1px", background: "#252525" }} />
          <span style={{ fontSize: "10px", color: "#444", letterSpacing: "1px" }}>OR</span>
          <div style={{ flex: 1, height: "1px", background: "#252525" }} />
        </div>
        {!restoreMode && (
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#e0e0e0", fontFamily: "'Space Mono', monospace" }}>
      ...
      <div style={{ borderBottom: "1px solid #141414", padding: "11px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#080808ee", backdropFilter: "blur(8px)", zIndex: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "5px" : "8px", flexWrap: "nowrap", justifyContent: "flex-end", overflow: "hidden" }}>
          ...
          {(!isMobile || view === "curriculum") && <button onClick={() => { setView("leaderboard"); window.scrollTo(0,0); }} style={{ background: view === "leaderboard" ? "#fbbf2420" : "none", border: "1px solid #1f1f1f", color: view === "leaderboard" ? "#fbbf24" : "#555", cursor: "pointer", padding: "4px 8px", borderRadius: "6px", fontSize: "11px", flexShrink: 0, whiteSpace: "nowrap" }}>{isMobile ? "🏆" : "🏆 Board"}</button>}
